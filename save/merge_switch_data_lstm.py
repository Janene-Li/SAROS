...
class programmableSwitch:
    ...
    def adjust_self_sampling_rate(self):
        ...
        if epoch_i < look_back - 1:
            ...
            hash_data = [[epoch_i, self.switch_id, len(self.packets_through), len(self.packets_sampled), x, self.sampled_max_hash, should_max_hash, should_max_hash-self.sampled_max_hash, next_p_threshold]]
...
def simulate(fattree_k):
    ...
    model = load_model("univ1_trace/model/univ1_[{}-{}]_epoch-{}_fattree{}_x{}.h5".format(file_begin, file_end, time_interval, fattree_k, 1000))
...
if __name__ == '__main__':
    ...
    # save dir
    save_data_dir = "univ1_trace/univ1_[{}-{}]_epoch{}_fattree{}/x{}_theta{}_lstm/".format(file_begin, file_end, time_interval, fattree_k, x, theta)
    ...
    logging.info("【lstm】 time interval = {}, fattree_k = {}, x = {}, theta = {} over".format(time_interval, fattree_k, x, theta))
