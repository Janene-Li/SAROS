...
class programmableSwitch:
    ...
    def sample_packets(self):
        ...
        self.packets_sampled = self.packets_through.head(x)
...
class physicalNetwork:
    ...
    def estimate_volume(self):
        ...
        # delta 0 : weight & sum 
        delta = 0.0
        weight = [1.0 / switch_num] * switch_num
        sum_switch_counter = 0
        for switch_id in range(switch_num):
            sum_switch_counter += len(self.networkSwitches[switch_id].packets_through)
        for switch_id in range(switch_num):
            weight[switch_id] = len(self.networkSwitches[switch_id].packets_through) / sum_switch_counter
            delta += self.networkSwitches[switch_id].delta * weight[switch_id] 
        
        # # delta 1 : harmonic mean 
        # delta_list = []
        # for switch_id in range(switch_num):
        ...
        logging.info("【TASK1】estimated volume = {}; actual volume = {}, ERROR = {}; effectice_sampled_packets_num = {}, max_hash = {}, should_max_hash={}".format(self.estimated_volume, actual_volume, 100 * error, k, h, should_x_hash))
...
if __name__ == '__main__':
    ...
    logging.info("【delta_weight】: time interval = {}, fattree_k = {}, x = {}, theta = {} over".format(time_interval, fattree_k, x, theta))
    ...
    # save dir
    save_data_dir = "univ1_trace/univ1_[{}-{}]_epoch{}_fattree{}/x{}_theta{}_delta_weight/".format(file_begin, file_end, time_interval, fattree_k, x, theta)
