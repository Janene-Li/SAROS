...
class programmableSwitch:
    ...
    def __init__(self, switch_id):
        ...
        self.pre_should_max_hash = 0.0
        self.should_max_hash = 0.0
        self.hash_diff_track = []
    ...
    def adjust_self_sampling_rate(self):
        ...
        self.should_max_hash = (x-1) / len(self.packets_through)

        hash_diff = self.should_max_hash - self.pre_should_max_hash
        self.hash_diff_track.append([hash_diff])

        self.pre_should_max_hash = self.should_max_hash
        ...
        if epoch_i < look_back - 1:
            ...
            hash_data = [[epoch_i, self.switch_id, len(self.packets_through), len(self.packets_sampled), x, self.sampled_max_hash, self.should_max_hash, self.should_max_hash-self.sampled_max_hash, next_p_threshold, hash_diff]]
            with open(hash_file_name, mode="a", newline="") as file:
                writer = csv.writer(file)
                writer.writerows(hash_data)
        
        else:
            self.hash_diff_track.pop(0)
...
class physicalNetwork:
    ...
    def estimate_volume(self):
        ...
        should_x_hash = 0.0
        if len(self.all_sampled_packet ) >= x:
            should_x_hash = self.all_sampled_packet.at[x-1, 'hash']
        else:
            len_all_sampled_hash = len(self.all_sampled_packet)
            should_x_hash = self.all_sampled_packet.at[len_all_sampled_hash-1, 'hash']
    ...
    def adjust_switch_sampling_rate(self, model):
        global epoch_i, look_back, alpha

        testX = []
        for switch_id in range(switch_num):
            self.networkSwitches[switch_id].adjust_self_sampling_rate()
            if epoch_i >= look_back - 1:
                testX.append(self.networkSwitches[switch_id].hash_diff_track) # [1, 2, - 10] [1, 2, - 10]...[1, 2, - 10]
        ...
        if epoch_i >= look_back - 1:
            ...
            for switch_id in range(switch_num):
                ...
                now_p_threshold = (x-1) / len(self.networkSwitches[switch_id].packets_through)  # epoch i: pi
                predict_p_threshold = testPredict[switch_id][0] + self.networkSwitches[switch_id].should_max_hash   # epoch i+1: ^pi+1
                next_p_threshold = predict_p_threshold
                if next_p_threshold <= 0:
                    next_p_threshold = (1 - alpha) * self.networkSwitches[switch_id].p_threshold + alpha * (x-1) / len(self.networkSwitches[switch_id].packets_through)
                self.networkSwitches[switch_id].p_threshold = next_p_threshold 

                logging.info("【TASK3】switch {}: old_p_threshold={} --> adjust to new_p_threshold={}; predict_diff={}".format(switch_id, now_p_threshold, next_p_threshold, testPredict[switch_id][0]))
                ...
                # write hash track file
                hash_data = [[epoch_i, switch_id, len_packets_through, len_packets_sampled, x, sample_max_hash, should_max_hash, should_max_hash-sample_max_hash, next_p_threshold, testPredict[switch_id][0]]]
                with open(hash_file_name, mode="a", newline="") as file:
                    writer = csv.writer(file)
                    writer.writerows(hash_data)
...
def simulate(fattree_k):
    ...
    model = load_model("univ1_trace/model/univ1_[{}-{}]_epoch-{}_fattree{}_x{}_diff.h5".format(file_begin, file_end, time_interval, fattree_k, 500))
...
def set_res_file():
    ...
    header = ["epoch","switch_id", "len_packets_through","len_packets_sampled", "x", "sample_max_hash", "should_max_hash", "should-actual", "predict_max_hash", "predict_diff"]
...
if __name__ == '__main__':
    ...
    # save dir
    save_data_dir = "univ1_trace/univ1_[{}-{}]_epoch{}_fattree{}/x{}_theta{}_diff/".format(file_begin, file_end, time_interval, fattree_k, x, theta)
    ...
    logging.info("【lstm diff】 time interval = {}, fattree_k = {}, x = {}, theta = {} over".format(time_interval, fattree_k, x, theta))
