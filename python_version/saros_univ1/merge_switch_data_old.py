...
class programmableSwitch:
    ...
    def adjust_self_sampling_rate(self):
        ...
        self.sample_max_hash = self.packets_sampled['hash'].max()
        ...
        if len(self.packets_through) >= x:
            should_max_hash = self.packets_through.hash.nsmallest(x).iloc[-1]# self.packets_through.at[sample_num-1, 'hash'] # 改了这里变成sample_num - 1
        else:
            should_max_hash = -1

        # actual k
        len_packets_sampled = len(self.packets_sampled)

        # estimated packets num
        self.should_max_hash_track.append([(x-1) / len(self.packets_through)])

        # now epoch
        now_p_threshold = self.p_threshold
        self.p_threshold = (1 - alpha) * self.p_threshold + alpha * (x-1) / len(self.packets_through) #     (x / ((actual_k - 1) / self.sample_max_hash)) 1000* (H/800)
        ...
        hash_data = [[epoch_i, self.switch_id, len(self.packets_through), len_packets_sampled, x, self.sample_max_hash, should_max_hash, should_max_hash-self.sample_max_hash, self.p_threshold]]
...
class physicalNetwork:
    ...
    def adjust_switch_sampling_rate(self):
...
def simulate(fattree_k):
    ...
    for epoch_i in range(epoch_len+1):
        ...
        network.adjust_switch_sampling_rate()
...
def set_res_file():
    ...
    evaluation file
    """
    evaluation_file_name = save_data_dir + "[volume estimation].csv"
...
if __name__ == '__main__':
    ...
    # save dir
    save_data_dir = "univ1_trace/univ1_[{}-{}]_epoch{}_fattree{}/x{}_theta{}_old/".format(file_begin, file_end, time_interval, fattree_k, x, theta)
    ...
    logging.info("old: time interval = {}, fattree_k = {}, x = {}, theta = {} over".format(time_interval, fattree_k, x, theta))
