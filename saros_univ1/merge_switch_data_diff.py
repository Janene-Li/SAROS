...
class programmableSwitch:
    ...
    def __init__(self, switch_id):
        ...
        self.should_max_hash_diff_track = []
    ...
    def adjust_self_sampling_rate(self):
        ...
        should_hash_diff = self.should_max_hash - self.pre_should_max_hash

        self.should_max_hash_diff_track.append([should_hash_diff])
        ...
        if epoch_i < look_back - 1:
            ...
            self.should_max_hash_diff_track.pop(0)
...
class physicalNetwork:
    ...
    def adjust_switch_sampling_rate(self, model):
        ...
        for switch_id in range(switch_num):
            ...
            if epoch_i >= look_back - 1:
                testX.append(self.networkSwitches[switch_id].should_max_hash_diff_track)
        ...
        if epoch_i >= look_back - 1:
            ...
            for switch_id in range(switch_num):
                ...
                self.networkSwitches[switch_id].p_threshold = testPredict[switch_id][0] + self.networkSwitches[switch_id].p_threshold
                ...
                #     self.networkSwitches[switch_id].should_max_hash = self.networkSwitches[switch_id].packets_through.hash.nsmallest(x).iloc[-1] # self.packets_through.at[sample_num-1, 'hash'] # 改了这里变成sample_num - 1
                # else:
                #     self.networkSwitches[switch_id].should_max_hash = -1
                ...
                hash_data = [[epoch_i, switch_id, len_packets_through, len_packets_sampled, x, sample_max_hash, self.networkSwitches[switch_id].should_max_hash, self.networkSwitches[switch_id].should_max_hash-sample_max_hash, next_p_threshold]]
...
def simulate(fattree_k):
    ...
    model = load_model("univ1_trace/univ1_[{}-{}]_epoch-{}_fattree{}_x{}_diff.h5".format(file_begin, file_end, time_interval, fattree_k, 1000))
