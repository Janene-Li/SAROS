...
control MyIngress(inout headers hdr,
    ...
    action bloom_hash(in bit<32> ipv4_src, in bit<32> ipv4_dst, in bit<16> srcport, in bit<16> dstport, in bit<8> protocol, in bit<16> checksum, in bit<32> seq_no, in bit<16> hdlen,out bit<32> h1, out bit<32> h2, out bit<32> h3) {
        hash(h1, HashAlgorithm.crc32, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,checksum,seq_no,hdlen}, 32w0x000fffff);
        hash(h2, HashAlgorithm.crc32_custom, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,checksum,seq_no,hdlen}, 32w0x000fffff);
        hash(h3, HashAlgorithm.identity, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,checksum,seq_no,hdlen}, 32w0x000fffff);
    }
    ...
    apply {
        ...
        if(flag==0)     standard_metadata.egress_spec =0;
    }
}
