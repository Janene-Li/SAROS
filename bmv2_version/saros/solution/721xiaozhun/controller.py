...
class myController(object):
    ...
    def recv_msg_cpu(self, pkt):
        ...
        if pkg not in self.mergelist:
            self.mergelist.append(pkg)
    ...
    def test (self): #for timeing and change T
        ...
        while 1:
            ...
            max_num = max(row[4] for row in networksample)

            if(x==1):
            	x=1;
		khash=2**32;
	    else:
            	khash=max_num    #networksample[-1][4]+1;
            logging.info('maxhash: %d', khash)
            logging.info('mergesize: %d', len(mergelist))
            ...
            for p4switch in self.topo.get_p4switches():
                ...
                if (len(p4switch)>4):
                    ...
                    continue;
                p_real=float(1000)/self.switchcounter[p4switch];
                if p_real>1:
                    p_real=0.99
