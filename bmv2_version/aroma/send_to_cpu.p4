...
control MyIngress(inout headers hdr,
    ...
    register<bit<32>>(Msize) kmvsketch;
    ...
    action bloom_hash(in bit<32> ipv4_src, in bit<32> ipv4_dst, in bit<16> srcport, in bit<16> dstport, in bit<8> protocol, in bit<16> checksum, in bit<32> seq_no, in bit<16> hdlen, out bit<32> h1, out bit<32> h2, out bit<32> h3) {
        hash(h1, HashAlgorithm.crc32, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,checksum,seq_no,hdlen}, 32w0x0002ffff);
        hash(h2, HashAlgorithm.crc16, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,checksum,seq_no,hdlen}, 32w0x0002ffff);
        hash(h3, HashAlgorithm.identity, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,checksum,seq_no,hdlen}, 32w0x0002ffff);
    }
    
    action kmv_hash(in bit<32> ipv4_src, in bit<32> ipv4_dst, in bit<16> srcport, in bit<16> dstport, in bit<8> protocol, in bit<16> checksum, in bit<32> seq_no, in bit<16> hdlen, out bit<32> khash) {
        hash(khash, HashAlgorithm.crc32, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,protocol,checksum,seq_no,hdlen}, Msize);
    }
    action kmv_hash1(in bit<32> ipv4_src, in bit<32> ipv4_dst, in bit<16> srcport, in bit<16> dstport, in bit<8> protocol, in bit<16> checksum, in bit<32> seq_no, in bit<16> hdlen, out bit<32> khash) {
        hash(khash, HashAlgorithm.crc32_custom, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,protocol,checksum,seq_no,hdlen}, 32w0xffffffffffffffff);
    }
    action kmv_hash2(in bit<32> ipv4_src, in bit<32> ipv4_dst, in bit<16> srcport, in bit<16> dstport, in bit<8> protocol, in bit<16> checksum, in bit<32> seq_no, in bit<16> hdlen, out bit<32> khash) {
        hash(khash, HashAlgorithm.crc32_custom, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,protocol,checksum,seq_no,hdlen}, 32w0xffffffffffffffff);
    }
    ...
    apply {
        ...
        if (hdr.tcp.isValid()) {
            ...
            kmv_hash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.src_port, hdr.tcp.dst_port,hdr.ipv4.protocol, hdr.tcp.checksum,hdr.tcp.seq_no,0, khash);
            kmv_hash1(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.src_port, hdr.tcp.dst_port,hdr.ipv4.protocol, hdr.tcp.checksum,hdr.tcp.seq_no,0, khash1);
            kmv_hash2(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.src_port, hdr.tcp.dst_port,hdr.ipv4.protocol, hdr.tcp.checksum,hdr.tcp.seq_no,0, khash2);
            routehash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.src_port, hdr.tcp.dst_port,hdr.ipv4.protocol, rhash);
            khash1[31:16]=khash2[31:16];
            meta.khash=khash;
            meta.khash1=khash1;
            ...
            kmv_hash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.udp.src_port, hdr.udp.dst_port, hdr.ipv4.protocol, hdr.udp.checksum, 0,hdr.udp.hdr_length, khash);
            kmv_hash1(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.udp.src_port, hdr.udp.dst_port,hdr.ipv4.protocol, hdr.udp.checksum, 0,hdr.udp.hdr_length, khash1);
            kmv_hash2(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.udp.src_port, hdr.udp.dst_port,hdr.ipv4.protocol, hdr.udp.checksum, 0,hdr.udp.hdr_length, khash2);
        ...
        if(bloom1 == epochnow && bloom3 == epochnow){
        ...
        bit<32> hashnow=0;
        bit<8> kmvflag;
        kmvsketch.read(hashnow,khash);
        if(khash1 == 0) khash1=1;
        if((hashnow>khash1 || hashnow==0)  && flag==1){
            kmvsketch.write(khash,khash1);
            meta.kmvflag = 1;
            meta.khash = khash;
            meta.khash1 = khash1;
        }else{
