...
control MyIngress(inout headers hdr,
    ...
    action bloom_hash(in bit<32> ipv4_src, in bit<32> ipv4_dst, in bit<16> srcport, in bit<16> dstport, in bit<8> protocol, in bit<16> checksum, out bit<32> h1, out bit<32> h2, out bit<32> h3) {
        hash(h1, HashAlgorithm.crc32, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,checksum}, 32w0x0000ffff);
        hash(h2, HashAlgorithm.crc16, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,checksum}, 32w0x0000ffff);
        hash(h3, HashAlgorithm.identity, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,checksum}, 32w0x0000ffff);
    }
    action kmv_hash(in bit<32> ipv4_src, in bit<32> ipv4_dst, in bit<16> srcport, in bit<16> dstport, in bit<8> protocol, in bit<16> checksum, out bit<32> khash) {
        hash(khash, HashAlgorithm.crc32, 32w0, {ipv4_src, ipv4_dst,srcport,dstport,protocol,checksum}, 32w0xffffffff);
    }
    ...
    apply {
        ...
        if (hdr.tcp.isValid()) {
            bloom_hash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.src_port, hdr.tcp.dst_port,hdr.ipv4.protocol, hdr.tcp.checksum, h1, h2, h3);
            kmv_hash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.src_port, hdr.tcp.dst_port,hdr.ipv4.protocol, hdr.tcp.checksum, khash);
        ...
        if (hdr.udp.isValid()) {
            //bloom_hash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.udp.src_port, hdr.udp.dst_port,hdr.ipv4.protocol, hdr.udp.checksum, h1, h2, h3);
            kmv_hash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.udp.src_port, hdr.udp.dst_port, hdr.ipv4.protocol, hdr.udp.checksum, khash);
        ...
        if (hdr.icmp.isValid()) {
            //bloom_hash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, 0, 0,hdr.ipv4.protocol, hdr.icmp.checksum, h1, h2, h3);
            kmv_hash(hdr.ipv4.srcAddr, hdr.ipv4.dstAddr,0, 0, hdr.ipv4.protocol, hdr.icmp.checksum, khash);
        ...
        if(bloom1 == epochnow && bloom2==epochnow && bloom3 ==epochnow){
